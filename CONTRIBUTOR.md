...
`main` does not accept a direct push. Open a pull request. The `pr-gate` workflow runs unit tests, integration tests, and the style check. If all three pass, the workflow approves and squash-merges a same-repo PR. If any fail, it requests changes and does not merge.
